...
'Carga información del archivo por subir vía FTP a estrados electrónicos y SINE
Private Sub CargaSubirArchivoFTP(iTarea As Integer)
Dim adors As New ADODB.Recordset, s As String, i As Byte
adors.Open "select f_Tarea_subirDocto(" & miActividad & "," & iTarea & ") from dual", gConSql, adOpenStatic, adLockReadOnly
If adors(0) > 0 Then 'Subir documento para estados electrónicos o Subir documento para SINE
    If Not cmdSubirDocto.Visible Then
        cmdSubirDocto.Visible = True
    End If
    If adors(0) = 1 Then 'Estrados
