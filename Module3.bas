...
CREATE OR REPLACE PROCEDURE SIAM.P_SeguimientoGuardaDatos(piSeg IN integer, piSegAnt in integer, piAna IN NUMBER, psFecha IN VARCHAR2, piAct in smallint, piTar in smallint, pires in smallint, piUsi IN smallint, pides smallint, psObs IN VARCHAR2, psDoctos IN VARCHAR2, psProg IN VARCHAR2, psAcuerdo in varchar2, psSancion in varchar2, piConSan in integer, psMemo in varchar2, pcur OUT sys_refcursor)
IS
...
BEGIN
   ...
   If Length(LTrim(psAcuerdo)) > 0 Then
        if i>0 then-- se actualiza
            update seguimientoacuerdos set acuerdo=psacuerdo where idseg=v_id;
        Else
            insert into seguimientoacuerdos (idseg,acuerdo,registro) values (v_id,psacuerdo,sysdate);
        end if;
   Else
       If i > 0 Then
   ...
   if instr(s,'|')>0 then
       ...
       if i>0 then-- se actualiza
           update seguimientosanción set fechainfracción=s_infraccion,iduni=s_unidad,monto=s_monto,importepesos=s_importe where idseg=v_id;
       Else
           s_oficio:=f_nuevofolio(4,0,piana);
           if instr(s_oficio,'???')>0 then
               s_oficio:=replace(s_oficio,'???',''||piconsan);
           end if;
           insert into seguimientosanción (idseg,año,consecutivo,oficio,fechainfracción,iduni,monto,importepesos,registro) values (v_id,f_folioanio(4,s_Oficio),f_folioconsecutivo(4,s_Oficio),s_oficio,s_infraccion,s_unidad,s_monto,s_importe,sysdate);
       end if;
   Else
       If i > 0 Then
 ...
 COMMIT;
    ...
    EXCEPTION
     WHEN NO_DATA_FOUND THEN
          ...
          sSen:=piSeg||'¬'||piSegAnt||'¬'||piAna||'¬'||psFecha||'¬'||piAct||'¬'||piTar||'¬'||piUsi||'¬'||psObs||'¬'||psDoctos||'¬'||psProg;
